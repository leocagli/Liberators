  ...
  return (
    <aside className="w-[212px] min-w-[212px] h-screen flex flex-col bg-[#07100a] border-r border-[#162816]">
      ...
      <div className="px-3 pt-5 pb-1">
        <p className="text-[10px] font-bold tracking-[0.15em] text-[#6a9e6e] uppercase mb-2.5 px-1">
          Agents
        </p>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className={`text-[12px] font-semibold truncate leading-tight ${isActive ? 'text-[#00f080]' : 'text-[#d4e8d4]'}`}>
                    {agent.name}
                  </p>
                  <div className="flex items-center gap-1 mt-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${agent.dotColor}`} />
                    <span className={`text-[10px] font-mono tracking-wider ${agent.statusColor}`}>
                      {agent.status}
                    </span>
                  </div>
                </div>
      ...
      {/* Navigation section */}
      <div className="px-3">
        <p className="text-[10px] font-bold tracking-[0.15em] text-[#6a9e6e] uppercase mb-2 px-1">
          Navigation
        </p>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-2 py-[7px] rounded-md text-left text-[12px] transition-all duration-150 ${
                  isActive
                    ? 'text-[#00f080] bg-[#0d1a0d]'
                    : 'text-[#a8cca8] hover:text-[#d4e8d4] hover:bg-[#0d180d]'
                }`}
              >
                <item.icon
                  size={13}
                  strokeWidth={isActive ? 2 : 1.5}
                  className={isActive ? 'text-[#00f080]' : 'text-[#6a9e6e]'}
                />
                {item.label}
                {isActive && (
      ...
      <div className="px-3 pb-4">
        <div className="border border-[#162816] rounded-md p-2.5 bg-[#0b1510]">
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <span className={`text-[10px] font-bold tracking-[0.12em] uppercase ${walletAddress ? 'text-[#00f080]' : 'text-[#6a9e6e]'}`}>
              {walletAddress ? 'Connected' : 'Wallet'}
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-mono text-[#a8cca8]">
              {compactWallet}
            </span>
            {walletAddress && (
              <button
